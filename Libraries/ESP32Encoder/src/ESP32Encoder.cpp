...
void ESP32Encoder::attach(int a, int b, enum encType et) {
	...
	r_enc_config.pos_mode = fullQuad ? PCNT_COUNT_DEC : PCNT_COUNT_DEC; //Count Only On Rising-Edges
	r_enc_config.neg_mode = PCNT_COUNT_DEC;   // Discard Falling-Edge

	r_enc_config.lctrl_mode = PCNT_MODE_REVERSE;    // Rising A on HIGH B = CW Step
	...
	if (et == full) {
		...
		r_enc_config.lctrl_mode = PCNT_MODE_KEEP;    // prior high mode is now low
		...
		r_enc_config.pos_mode = PCNT_COUNT_DEC; //disabling channel 1
		r_enc_config.neg_mode = PCNT_COUNT_DEC;   // disabling channel 1

		r_enc_config.lctrl_mode = PCNT_MODE_KEEP;    // disabling channel 1
		r_enc_config.hctrl_mode = PCNT_MODE_KEEP;    // disabling channel 1
